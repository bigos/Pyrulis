...
(defvar *last-column-letter* (subseq *board-column-letters* (- (parse-integer (sgf-analyser:header-value "SZ")) 1)))
...
(defun parse-board-coordinates (str)
  (let* ((column (position (char str 0 ) (subseq *board-column-letters* 0 sgf-analyser:*board-size*) :test #'equal)) 
	 (row (- (parse-integer (sgf-analyser:header-value "SZ")) (parse-integer (subseq str 1)))))
    (cons column row)))
